...
import type { CreateUserBody, UpdateUserBody, User } from "@/api/user/userModel";
...
import { comparePasswords, hashPassword } from "@/common/utils/security";
...
export class UserService {
	...
	async verifyEmailChange(token: string): Promise<ServiceResponse<Omit<User, "password"> | null>> {
		try {
			...
			const verificationToken = await this.userRepository.findVerificationTokenAsync(
				token,
				"CHANGE_EMAIL",
			);
			if (
