...
export class UploadController {
  ...
  public async uploadSingleFile(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res
        ...
        .json(ServiceResponse.failure('Failed to upload file.', null, StatusCodes.INTERNAL_SERVER_ERROR));
      return; // Explicitly return void
    }
  }
  ...
  public async uploadMultipleFiles(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      res.status(StatusCodes.OK).json(ServiceResponse.success('Files uploaded successfully.', uploadedFiles));
      return; // Explicitly return void
    } catch (error) {
      logger.error({ error }, 'Error during multiple file upload.');
      res
        .status(StatusCodes.INTERNAL_SERVER_ERROR)
        .json(ServiceResponse.failure('Failed to upload files.', null, StatusCodes.INTERNAL_SERVER_ERROR));
      return; // Explicitly return void
    }
  }
}
