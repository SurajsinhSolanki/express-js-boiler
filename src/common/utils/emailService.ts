...
export const sendEmail = async (options: EmailOptions) => {
	try {
		...
		logger.info(
			`Email sent to ${options.to} with subject: ${options.subject} using template: ${options.template}`,
		);
	} catch (error) {
		logger.error(
			{ error },
			`Failed to send email to ${options.to} using template: ${options.template}`,
		);
