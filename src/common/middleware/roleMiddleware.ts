...
// Define roles (can be extended with more granular permissions)
...
export const authorizeRoles = (allowedRoles: UserRole[]) => {
  return (req: Request, res: Response, next: NextFunction) => {
    if (!req.user) {
      logger.warn('Authorization failed: No user found in request (authMiddleware likely missing or failed).');
      res
        .status(StatusCodes.UNAUTHORIZED)
        .json(ServiceResponse.failure('Unauthorized: Authentication required.', null, StatusCodes.UNAUTHORIZED));
    ...
    const userHasRequiredRole = allowedRoles.some(role => {
      ...
      if (role === UserRole.USER && (req.user?.role === UserRole.USER || req.user?.role === UserRole.ADMIN)) {
        return true;
      }
      return false;
    });
    ...
    if (userHasRequiredRole) {
      ...
      res
        ...
        .json(ServiceResponse.failure('Forbidden: Insufficient permissions.', null, StatusCodes.FORBIDDEN));
      return; // Explicitly return void
    }
  };
};
